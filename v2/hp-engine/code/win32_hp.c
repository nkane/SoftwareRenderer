...
// TODO(nick): offset amounts of definitately fucking this up ...
#if 1
    ...
    for (int i = scanlineStart; i < scanlineEnd; i++) 
    {
        ...
        //int32 xLowOffset = point1.X;
        //int32 xHighOffset = point2.X;
...
RenderTriangle(win32_offscreen_buffer buffer, v2f point1, v2f point2, v2f point3, float red, float green, float blue, float alpha)
{
    ...
    if (scanlineEnd != scanlineStart)
    {
        ...
        float slope1 = vector1.X / vector1.Y;
        float slope2 = vector2.X / vector2.Y;
        RenderHalfTriangle(GlobalBackBuffer, scanlineStart, scanlineEnd, point1, slope1, point2, slope2);
    }
    ...
    if (scanlineStart != scanlineEnd)
    {
        float slope1 = vector1.X / vector1.Y;
        float slope2 = vector2.X / vector2.Y;
        RenderHalfTriangle(GlobalBackBuffer, start1.X, start1.Y, slope1, start2.X, start2.Y, slope2, scanlineStart, scanlineEnd);
    }
    */
}
    ...
    if (RegisterClassA(&windowClass))
    {
        ...
        if (GlobalWindowHandle) 
        {
            ...
            while (GlobalRunning)
            {
                ...
                RenderHalfTriangle(GlobalBackBuffer, 100, 200, trianglePoint1, 1.0f, trianglePoint2, 0.1f);
                */
